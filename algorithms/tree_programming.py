...
class BinaryTree(Stack):
    ...
    def height_of_tree(self, start):    # Maximum Depth of Binary Tree
        if start is None:
            return -1
    ...
    def size_of_tree_reversal(self, start):  # Count Complete Tree Nodes
        if start is None:
            return 0
    ...
    def are_identical_tree(self, root1, root2):     # Same Tree
    ...
    def chk_binary(self, root, index, no_of_nodes):
        ...
        return self.chk_binary(root.left, 2*index+1, no_of_nodes) and \
               self.chk_binary(root.right, 2*index+2, no_of_nodes)
